...
import { SiLoopback, SiLoop } from "react-icons/si";
...
const App = () => {
  ...
  const handlePlay = (id, title, img) => { playSong(id, title, img, setLoading, setIsLoading, audioRef, setCurrentSong, setAudioUrl, setIsPlaying, currentSong, setPreviousSongs, setPlayingRN, isPlayingRN); };
...
const handlePlayNow = (id, title) => {
   handlePlay(id, title, img)
  setQueue(queue.filter((song) => song.videoId !== id));
};
    ...
    <div className='app'>
      ...
      {searchResults.length > 0 && (
        <>
          <div className='nav'>
            <h2
              className={activeTab === 'searchResults' ? 'active-tab' : ''}
              onClick={() => setActiveTab('searchResults')}>
              Search Results
            </h2>
            <h2
              className={activeTab === 'upnext' ? 'active-tab' : ''}
                ...
                <div>
                  {searchResults.map((result) => (
                    <div key={result.videoId} className="result-item">
                      ...
                      <button className="play-button" onClick={() => setQueue([...queue, { ...result, addedBy: "user" }])}>Add to Queue</button>
                      <button className="play-button" onClick={() => handlePlay(result.videoId, result.title, result.thumbnail)} >Play</button>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div>
                  {queue.map((result) => (
                    <div key={result.videoId} className="result-item">
                      ...
                      <button className="play-button" onClick={() => handlePlayNow(result.videoId, result.title, result.thumbnail)} >Play Now</button>
                      <button className="play-button" onClick={() => setQueue(queue.filter((song) => song.videoId !== result.videoId))}>Remove</button>
                    </div>
                  ))}
                </div>
              )}
      ...
      {isPlaying && (
        <div className="play-container">
          <div className="current-play-container">
           ...
           <img src={currentSong.img} alt={currentSong.title} className='current-img' />
